...
def avg_img(image):
    return np.sum(image)*100.0/len(image.reshape(-1,))

def plot(hist, epochnum, batchnum, name, is_attnnet=0,model_name = 'u_net'):
    result_images_path='../data/results/images/'
    plt.figure()
...
#         val_acc = hist['val_dice_coef']
    ...
    plt.savefig(result_images_path+model_name+'_Loss.png')
    ...
    plt.savefig(result_images_path+model_name+'_Dice.png')
